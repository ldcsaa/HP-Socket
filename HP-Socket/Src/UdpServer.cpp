...
const DWORD	CUdpServer::DEFAULT_WORKER_THREAD_COUNT		= min((::SysGetNumberOfProcessors() * 2 + 2), MAX_WORKER_THREAD_COUNT);
const DWORD CUdpServer::DEFAULT_FREE_SOCKETOBJ_LOCK_TIME= 10 * 1000;
...
void CUdpServer::DetectConnections()
{
	...
	for(TUdpSocketObjPtrMapCI it = m_mpClientSocket.begin(); it != m_mpClientSocket.end(); ++it)
	{
		DWORD detectFails = ::InterlockedIncrement(&it->second->detectFails);

		if(detectFails >= m_dwDetectAttempts)
