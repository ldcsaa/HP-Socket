...
void CServerDlg::OnBnClickedStart()
{
	...
	m_Server->SetTurnoffCongestCtrl(TRUE);
	m_Server->SetFlushInterval(10);
	m_Server->SetResendByAcks(2);
	m_Server->SetMinRto(30);
	*/
...
EnHandleResult CServerDlg::OnPrepareListen(IUdpServer* pSender, SOCKET soListen)
{
	...
	VERIFY(::SYS_SSO_RecvBuffSize(soListen, 4 * 1024 * 1024) == NO_ERROR);
...
EnHandleResult CServerDlg::OnHandShake(IUdpServer* pSender, CONNID dwConnID)
{
	if(m_lClientCount == 0)
	{
