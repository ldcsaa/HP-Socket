...
void CClientDlg::OnBnClickedStart()
{
	...
	{
		smart_simple_ptr<CUdpArqClientPtr> pSocket = new CUdpArqClientPtr(this);
		...
		*/
		...
		(*pSocket)->SetTurnoffCongestCtrl(TRUE);
		(*pSocket)->SetFlushInterval(10);
		(*pSocket)->SetResendByAcks(2);
		(*pSocket)->SetMinRto(30);
		*/
	...
	for(int i = 0; i < m_iTestTimes; i++)
	{
		...
		{
			CUdpArqClientPtr* pSocket = m_vtClients[j];
...
void CClientDlg::OnBnClickedStop()
{
	...
	{
		CUdpArqClientPtr* pSocket = m_vtClients[i];
...
EnHandleResult CClientDlg::OnHandShake(IUdpClient* pSender, CONNID dwConnID)
{
