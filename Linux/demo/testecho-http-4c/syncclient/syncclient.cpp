...
HP_HttpSyncClient s_http_client;
HP_HttpsSyncClient s_https_client;
HP_HttpSyncClient s_pClient;
...
CStringA GetHeaderSummary(HP_HttpSyncClient pSender, LPCSTR lpszSep, int iSepCount, BOOL bWithContentLength)
{
	CStringA strSEP1;
...
EnHandleResult __HP_CALL OnConnect(HP_HttpSyncClient pSender, CONNID dwConnID)
{
	...
	::PostOnConnect2(dwConnID, szAddress, usPort, s_sender_names[pSender]);

	return HR_OK;
}

EnHandleResult __HP_CALL OnHandShake(HP_HttpSyncClient pSender, CONNID dwConnID)
{
...
EnHandleResult __HP_CALL OnSend(HP_HttpSyncClient pSender, CONNID dwConnID, const BYTE* pData, int iLength)
{
	::PostOnSend(dwConnID, pData, iLength, s_sender_names[pSender]);
	return HR_OK;
}

EnHandleResult __HP_CALL OnReceive(HP_HttpSyncClient pSender, CONNID dwConnID, const BYTE* pData, int iLength)
{
	::PostOnReceive(dwConnID, pData, iLength, s_sender_names[pSender]);
	return HR_OK;
}

EnHandleResult __HP_CALL OnClose(HP_HttpSyncClient pSender, CONNID dwConnID, EnSocketOperation enOperation, int iErrorCode)
{
...
EnHttpParseResult __HP_CALL OnMessageBegin(HP_HttpSyncClient pSender, CONNID dwConnID)
{
	::PostOnMessageBegin(dwConnID, s_sender_names[pSender]);
	return HPR_OK;
}

EnHttpParseResult __HP_CALL OnStatusLine(HP_HttpSyncClient pSender, CONNID dwConnID, USHORT usStatusCode, LPCSTR lpszDesc)
{
	::PostOnStatusLine(dwConnID, usStatusCode, lpszDesc, s_sender_names[pSender]);
	return HPR_OK;
}

EnHttpParseResult __HP_CALL OnHeader(HP_HttpSyncClient pSender, CONNID dwConnID, LPCSTR lpszName, LPCSTR lpszValue)
{
	::PostOnHeader(dwConnID, lpszName, lpszValue, s_sender_names[pSender]);
	return HPR_OK;
}

EnHttpParseResult __HP_CALL OnHeadersComplete(HP_HttpSyncClient pSender, CONNID dwConnID)
{
...
EnHttpParseResult __HP_CALL OnBody(HP_HttpSyncClient pSender, CONNID dwConnID, const BYTE* pData, int iLength)
{
	::PostOnBody(dwConnID, pData, iLength, s_sender_names[pSender]);
	return HPR_OK;
}

EnHttpParseResult __HP_CALL OnChunkHeader(HP_HttpSyncClient pSender, CONNID dwConnID, int iLength)
{
	::PostOnChunkHeader(dwConnID, iLength, s_sender_names[pSender]);
	return HPR_OK;
}

EnHttpParseResult __HP_CALL OnChunkComplete(HP_HttpSyncClient pSender, CONNID dwConnID)
{
	::PostOnChunkComplete(dwConnID, s_sender_names[pSender]);
	return HPR_OK;
}

EnHttpParseResult __HP_CALL OnMessageComplete(HP_HttpSyncClient pSender, CONNID dwConnID)
{
...
EnHttpParseResult __HP_CALL OnUpgrade(HP_HttpSyncClient pSender, CONNID dwConnID, EnHttpUpgradeType enUpgradeType)
{
...
EnHttpParseResult __HP_CALL OnParseError(HP_HttpSyncClient pSender, CONNID dwConnID, int iErrorCode, LPCSTR lpszErrorDesc)
{
...
EnHandleResult __HP_CALL OnWSMessageHeader(HP_HttpSyncClient pSender, CONNID dwConnID, BOOL bFinal, BYTE iReserved, BYTE iOperationCode, const BYTE lpszMask[4], ULONGLONG ullBodyLen)
{
...
EnHandleResult __HP_CALL OnWSMessageBody(HP_HttpSyncClient pSender, CONNID dwConnID, const BYTE* pData, int iLength)
{
...
EnHandleResult __HP_CALL OnWSMessageComplete(HP_HttpSyncClient pSender, CONNID dwConnID)
{
...
void SendHttp(CHttpCommandParser* pParser)
{
	...
	CString strContent;
	strContent.Format(_T("[%s] %s://%s:%u %s"), (LPCTSTR)pParser->m_strMethod, s_sender_names[s_pClient], (LPCTSTR)g_app_arg.remote_addr, s_usPort, (LPCTSTR)pParser->m_strPath);
	::LogSending(dwConnID, strContent, s_sender_names[s_pClient]);
	...
	if(!isOK)
	{
		::LogSendFail(dwConnID, ::SYS_GetLastError(), ::SYS_GetLastErrorStr(), s_sender_names[s_pClient]);
		return;
	}
	...
	::HP_HttpSyncClient_GetResponseBody(s_pClient, &pData, &iLength);

	if(iLength > 0)
	{
		LPCSTR lpszEnc = ::HP_HttpClient_GetContentEncoding(s_pClient);
		...
		if(lpszEnc && ::StrStr(lpszEnc, "gzip"))
		{
			...
			if(rs == 0)
				::PostUncompressBody(dwConnID, dwLen, s_sender_names[s_pClient]);
			else
			{
				::PostUncompressBodyFail(dwConnID, rs, s_sender_names[s_pClient]);

				::HP_Client_Stop(s_pClient);
			}
		}
	}
}
...
void SendWebSocket(CHttpCommandParser* pParser)
{
	...
	VERIFY(::HP_HttpClient_GetWSMessageState(s_pClient, &bFinal, &iReserved, &iOperationCode, &lpszMask, &ullBodyLen, nullptr));
	...
	if(ullBodyLen > 0)
	{
		...
		::HP_HttpSyncClient_GetResponseBody(s_pClient, (LPCBYTE*)&pData, &iLength);
	}

	if(iOperationCode == 0x8)
	{
		::HP_HttpClient_SendWSMessage(s_pClient, TRUE, 0, 0x8, HTTP_WEB_SOCKET_MASK_KEY, nullptr, 0, 0);
		::HP_Client_Stop(s_pClient);
	}
}
...
void OnCmdStart(CHttpCommandParser* pParser)
{
	...
	if(!::HP_Client_StartWithBindAddress(s_pClient, g_app_arg.remote_addr, s_usPort, g_app_arg.async, g_app_arg.bind_addr))
		::LogClientStartFail(::HP_Client_GetLastError(s_pClient), ::HP_Client_GetLastErrorDesc(s_pClient), s_sender_names[s_pClient]);
}

void OnCmdStop(CHttpCommandParser* pParser)
{
	if(::HP_Client_Stop(s_pClient))
		::LogClientStop(s_sender_names[s_pClient]);
	else
		::LogClientStopFail(::HP_Client_GetLastError(s_pClient), ::HP_Client_GetLastErrorDesc(s_pClient), s_sender_names[s_pClient]);
}

void OnCmdStatus(CHttpCommandParser* pParser)
{
	pParser->PrintStatus(::HP_Client_GetState(s_pClient), s_sender_names[s_pClient]);
}
...
void OnCmdSend(CHttpCommandParser* pParser)
{
	if(!::HP_Client_HasStarted(s_pClient))
	{
		::LogSendFail(::HP_Client_GetConnectionID(s_pClient), SE_ILLEGAL_STATE, ::HP_GetSocketErrorDesc(SE_ILLEGAL_STATE), s_sender_names[s_pClient]);
		return;
	}

	::HP_HttpClient_GetUpgradeType(s_pClient) == HUT_WEB_SOCKET ? SendWebSocket(pParser) : SendHttp(pParser);
	::HP_HttpSyncClient_CleanupRequestResult(s_pClient);
}

void OnCmdOpenUrl(CHttpCommandParser* pParser)
{
	LPCTSTR lpszName			= pParser->m_bHttps ? HTTPS_NAME : HTTP_NAME;
	HP_HttpSyncClient pClient	= pParser->m_bHttps ? s_https_client : s_http_client;
	...
	::LogSending(::HP_Client_GetConnectionID(pClient), strContent, lpszName);
	...
	if(!::HP_HttpSyncClient_OpenUrl(pClient, pParser->m_strMethod, pParser->m_strPath, szHeaders.get(), iSize, pBody, iLength, pParser->m_bFlag))
	{
		::LogSendFail(::HP_Client_GetConnectionID(pClient), ::SYS_GetLastError(), ::SYS_GetLastErrorStr(), lpszName);
		return;
	}
	...
	::HP_HttpSyncClient_GetResponseBody(pClient, &pBody, &iLength);

	if(iLength > 0)
	{
		LPCSTR lpszEnc = ::HP_HttpClient_GetContentEncoding(pClient);

		if(lpszEnc && ::StrStr(lpszEnc, "gzip"))
		{
			int rs		= 0;
			DWORD dwLen	= ::SYS_GZipGuessUncompressBound(pBody, iLength);
			...
			{
				CBufferPtr szBuff(dwLen);
				rs = ::SYS_GZipUncompress(pBody, iLength, szBuff, &dwLen);
			}

			if(rs == 0)
				::PostUncompressBody(::HP_Client_GetConnectionID(pClient), dwLen, lpszName);
			else
			{
				::PostUncompressBodyFail(::HP_Client_GetConnectionID(pClient), rs, lpszName);

				::HP_Client_Stop(pClient);
			}
		}
	}
}
...
void OnCmdPause(CHttpCommandParser* pParser)
{
	if(::HP_Client_PauseReceive(s_pClient, pParser->m_bFlag))
		::LogPause(::HP_Client_GetConnectionID(s_pClient), pParser->m_bFlag, s_sender_names[s_pClient]);
	else
		::LogPauseFail(::HP_Client_GetConnectionID(s_pClient), pParser->m_bFlag, s_sender_names[s_pClient]);
}
...
BOOL CreateHPSocketObjects()
{
	...
	s_http_client	= ::Create_HP_HttpSyncClient(s_http_listener);
	s_https_client	= ::Create_HP_HttpsSyncClient(s_http_listener);
	...
	s_sender_names[s_http_client]	= HTTP_NAME;
	s_sender_names[s_https_client]	= HTTPS_NAME;
