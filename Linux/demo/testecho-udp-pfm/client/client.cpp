...
public:
	...
	{
		VERIFY(::SSO_RecvBuffSize(socket, 256 * 1024) == NO_ERROR);
...
void OnCmdStart(CCommandParser* pParser)
{
	...
	{
		unique_ptr<IUdpClient> pSocket = make_unique<CUdpClient>(&s_listener);
	...
	for(DWORD i = 0; i < g_app_arg.test_times; i++)
	{
		...
		{
			IUdpClient* pSocket = s_vtClients[j];
